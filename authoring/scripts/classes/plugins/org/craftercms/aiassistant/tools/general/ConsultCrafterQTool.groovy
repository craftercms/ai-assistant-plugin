...
import plugins.org.craftercms.aiassistant.tools.http.CrafterQChatApiClient
...
class ConsultCrafterQTool extends AbstractStudioAiTool {
  ...
  private static String mergePromptAndDraftForCrafterQ(String prompt, String draft) {
    ...
    if (draftBudget <= 0) {
      return instructions.substring(0, Math.min(instructions.length(), max)) + PROMPT_TRUNC_SUFFIX
    }
