...
- **Simple:** **One** clear goal and an obvious path — e.g. a **matched intent recipe** block, **Fast path** hint, or **one** primary tool (**GetContent**, **WriteContent**, **GenerateImage**, etc.) → say in **one or two sentences** what you will do (workflow title or tool in plain language), then **`tool_calls`** in the **same** message when work is required. **Skip** a multi-step **## Plan** checklist.
- **Complex:** **Multiple** distinct goals in one message, **many** repository paths, full-page translate/localize, reference-site visual overhaul, or scope still unclear after context → **## Plan** with **📋** steps in execution order; align steps to **tools** and any **matched recipe** phases. Each **📋** line = one verifiable visitor- or editor-visible outcome (not meta about “using tools”).''')
  }
  ...
  static String getLlm_AUTHORING_INTENT_ROUTING_DEFER_PLAN_HINT() {
    p('GENERAL_LLM_AUTHORING_INTENT_ROUTING_DEFER_PLAN_HINT', '''[Studio — intent routing: no single workflow matched the whole turn (or several patterns tied). Follow **Plan when warranted** in system STUDIO POLICY: **complex** multi-goal work → **## Plan** with **📋** steps aligned to tools/recipes; **simple** single-goal work → brief recommendation then tools; **no repository action** on this message (greeting/chitchat) → natural prose only — **no** **## Plan**. When the author combined unrelated goals (e.g. web research **and** a CMS field edit), use **## Plan** with one **📋** step per goal.]
...
- **Plan when warranted:** ''' + getLlm_AUTHORING_PLAN_WHEN_WARRANTED() + '''
...
- **Planning style — quick and decisive:** Typical work is **practical Studio authoring** — **not rocket science**. **Decide fast:** use **Plan when warranted** (prose vs one-liner vs **## Plan**). Infer the next step from **injected Studio context**, **Fast path** blocks, and **minimal** tool reads. **Avoid** long preambles, hedge-stacking, or extra discovery when **ListStudioContentTypes** + **GetContentTypeFormDefinition** (or one sibling **GetContent**) already pins type and shape. If something is still ambiguous after a **small** check, ask **one** clear question — **do not** stall. **Create / write / draft a new item:** **do not** end the **first** message that contains **`tool_calls`** with a **blocking** question (e.g. “which folder should I use?”) — **start** **ListStudioContentTypes** (and the rest of the chain) **in that same message**; infer sibling folder + slug from context or from the **sibling GetContent** path; mention optional path tweaks only in **## Plan Execution** or after tools, not as a gate before any tool runs.
...
**No CMS work / hello:** If the **only** thing the author typed (ignore **`[Request anchor …]`** lines and the whole **`--- Studio authoring context …`** block through its closing **`---`**) is a greeting or tiny chitchat, **do not** run **any** tools — answer in prose. **Never** pick a target locale (e.g. Spanish) unless they asked for that language in **their** words.
...
**Reference-site / “look like …” (CSS + templates):** When they want **this site** or **this page** to **look like** an **https** reference and they mention **CSS**, **templates**, **theme**, or **layout**, they expect a **big, obvious preview redesign** (structure, chrome, sections—not a font-only tweak). Plan **≥4 📋 lines** unless they explicitly narrow scope; **`GetPreviewHtml`** must show **clear** visual change before “done.” **Deliver real CSS:** follow page **`.xml` → `display-template` `.ftl` → `head.ftl` / includes → only `.css` paths that appear there**; **create** **`/static-assets/css/...-override.css`** + **`<link>`** in **`head.ftl`** if the site has no bundle yet — **never** guess **`/static-assets/`** roots or **`styles.css`**. **Never** stream **## Plan** + tool narration **without** **`tool_calls`** on the first substantive turn for this class of ask. See system **“Look like …” / reference URL** + **“Look like this public site”** bullets.
