...
import plugins.org.craftercms.aiassistant.contrib.tool.builtin.cms.internal.CmsListPagesAndComponents
...
import plugins.org.craftercms.aiassistant.contrib.tool.builtin.cms.internal.CmsResearchSiteContent
...
private AuthoringIntentRecipeEngine() {}
  ...
  private static final Set<String> READ_ONLY_TOOLS =
    plugins.org.craftercms.aiassistant.engine.catalog.StudioAiToolRegistry.recipeEngineReadOnlyWireNames()
   ...
   *   confirmation tool via {@link plugins.org.craftercms.aiassistant.engine.catalog.StudioAiToolRegistry#mergeRecipeConfirmationArgs}
  ...
  static Map runConfirmationStepsBlock(
    ...
    Set<String> confirmationWires =
      plugins.org.craftercms.aiassistant.engine.catalog.StudioAiToolRegistry.recipeEngineConfirmationWireNames()
    Map bindings = ops.recipeEngineAuthoringBindings()
    List<Map> stepSummaries = new ArrayList<>()
    List<Map> stepResults = new ArrayList<>()
    Map<String, Map> namedSoFar = new LinkedHashMap<>()
    ...
    for (Object stepObj : steps) {
      ...
      resolvedArgs = coerceConfirmationArgStrings(
        plugins.org.craftercms.aiassistant.engine.catalog.StudioAiToolRegistry.mergeRecipeConfirmationArgs(
          tool,
          resolvedArgs,
  ...
  /**
   * Confirmation tool progress path hint.
   * @param tool Caller-supplied input.
  ...
  private static Map executeReadOnlyTool(StudioToolOperations ops, String tool, Map input) {
    ...
    return plugins.org.craftercms.aiassistant.engine.catalog.StudioAiToolRegistry.executeRecipePrefetchTool(
      tool,
      args,
      ops
    )
  ...
  /** Delegates to {@link plugins.org.craftercms.aiassistant.engine.catalog.StudioAiToolRegistry#executeRecipeConfirmationTool}. */
  private static Map executeConfirmationTool(StudioToolOperations ops, String tool, Map input) {
    return plugins.org.craftercms.aiassistant.engine.catalog.StudioAiToolRegistry.executeRecipeConfirmationTool(
      tool,
      (Map) (input ?: [:]),
      ops
    )
  ...
  /**
   * Form definition xml from prefetch bindings.
   * @param parsed Caller-supplied input.
   * @return Text result, or empty or null when unavailable.
   */
  ...
  /**
   * Form definition xml from prefetch steps.
   * @param parsed Caller-supplied input.
   * @return Text result, or empty or null when unavailable.
   */
  ...
  /**
   * Form definition xml from tool result.
   * @param resultObj Caller-supplied input.
   * @return Text result, or empty or null when unavailable.
   */
  ...
  /**
   * True when prefetch JSON includes two or more successful {@code GetContent} paths (shape reference), not only
   * a single anchor/context read.
   */
  private static boolean prefetchIncludesSiblingShapeGetContent(String prefetchBlock) {
    Object parsed = parsePrefetchJsonEnvelope(prefetchBlock)
    if (!(parsed instanceof Map)) {
      return false
    }
    Object stepsObj = ((Map) parsed).get('steps')
    ...
    for (Object stepObj : (List) stepsObj) {
      ...
      Map res = (Map) resObj
      Object cxObj = res.get('contentXml')
      if (!(cxObj instanceof String) || !((String) cxObj).trim() || prefetchFieldLooksTruncated((String) cxObj)) {
        continue
      }
      String p = (res.get('path') ?: res.get('contentPath') ?: '')?.toString()?.trim()
      if (p) {
        paths.add(p.toLowerCase(Locale.ROOT))
      }
    }
    return paths.size() >= 2
  }
  ...
  static Map buildNewContentItemSiblingReadDirective(String prefetchBlock) {
    ...
    if (prefetchIncludesSiblingShapeGetContent((prefetchBlock ?: '').toString())) {
  ...
  /**
   * Prior derived root field values from draft.
   * @param prior Caller-supplied input.
  ...
  /**
   * Node selector candidate binding names.
   * @param supplementConfig Caller-supplied input.
  ...
  /**
   * Node selector catalog filter from config.
   * @param supplementConfig Caller-supplied input.
   * @return Map payload for tools or orchestration.
   */
  ...
  /**
   * Node selector research fallback from config.
   * @param supplementConfig Caller-supplied input.
   * @return Map payload for tools or orchestration.
   */
  ...
  /**
   * Node selector research queries.
   * @param fallback Caller-supplied input.
   * @param priorAuthorLabel Caller-supplied input.
   * @return List<String> result.
   */
  ...
  /**
   * Prefetch initial bindings from supplement.
   * @param supplementConfig Caller-supplied input.
   * @return Map payload for tools or orchestration.
   */
  ...
  /**
   * Parse first prefetch json block.
   * @param prefetchBlock Caller-supplied input.
  ...
  /**
   * Compact research hits.
   * @param researchResult Caller-supplied input.
  ...
  /**
   * Builds create from prior draft hotpath for tool or orchestration output.
   * @param wirePrompt Caller-supplied input.
   * @param supplemental Caller-supplied input.
   * @return Text result, or empty or null when unavailable.
   */
  ...
  /**
   * Exact catalog match content type id.
   * @param typeRows Caller-supplied input.
   * @param authorTypePhrase Caller-supplied input.
   * @return Text result, or empty or null when unavailable.
   */
  ...
  /**
   * Normalizes and validates catalog match phrase; throws when required values are missing.
  ...
  /**
   * Extracts quick create path template from repository XML or related text.
   * @param formDefinitionXml Caller-supplied input.
   * @return Text result, or empty or null when unavailable.
   */
  ...
  /**
   * Quick create path to search prefix.
   * @param quickCreateTemplate Caller-supplied input.
   * @return Text result, or empty or null when unavailable.
   */
  ...
  /**
   * Catalog type tail for research.
  ...
  /**
   * Suggest new item path from quick create.
   * @param quickCreateTemplate Caller-supplied input.
  ...
  /**
   * Slugify for repository file name.
  ...
  private static String formatContentFieldInnerXml(String fieldId, String plain) {
    ...
    if (fieldId.endsWith('_html')) {
      ...
      return ContentSubgraphAggregator.wrapCdataBody('<p>' + escapeXmlElementText(raw) + '</p>')
    }
  ...
  private static String formatRteCdataFromPlainText(String plain) {
    ...
    return ContentSubgraphAggregator.wrapCdataBody(sb.toString())
  }
