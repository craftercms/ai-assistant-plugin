...
class StudioToolOperations {
  ...
  // Preview/HTTP URL helpers: plugins.org.craftercms.aiassistant.studio.repository.StudioToolOperationsSupport
  ...
  Map writeRepositoryFile(String siteId, String fullPath, byte[] bytes, boolean unlockAfterWrite = true) {
    ...
    if (!notified) {
      log.warn('writeRepoFile: content saved but preview refresh event failed for siteId={} path={}', siteId, fullPath)
    }
  ...
  /**
   * Normalizes and validates leading slash; throws when required values are missing.
  ...
  /**
   * Lists studio sandbox child folder names via content service for the model or author.
   * @param siteId Studio or repository context for this call.
   * @param parentSandboxRepoPath Caller-supplied input.
  ...
  /**
   * Extracts first level folder names from get children result from repository XML or related text.
