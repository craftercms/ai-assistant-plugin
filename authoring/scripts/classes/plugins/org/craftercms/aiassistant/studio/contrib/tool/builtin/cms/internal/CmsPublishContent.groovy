...
private CmsPublishContent() {}
  ...
  static Map fromToolInput(StudioToolOperations ops, Map input, boolean pathProtectFormItem = false, String normProtectedFormPath = '') {
    ...
    if (!scope) {
      if (plugins.org.craftercms.aiassistant.studio.engine.context.AuthoringPreviewContext.isTruthy(input?.publishEntireSite) ||
        plugins.org.craftercms.aiassistant.studio.engine.context.AuthoringPreviewContext.isTruthy(input?.publishAll) ||
        plugins.org.craftercms.aiassistant.studio.engine.context.AuthoringPreviewContext.isTruthy(input?.publishEverything)) {
        scope = 'all'
    ...
    if (scope == 'paths') {
      ...
      def publishable = []
      for (def p : paths) {
        if (pathProtectFormItem && plugins.org.craftercms.aiassistant.studio.engine.context.AuthoringPreviewContext.sameRepoPath(p, normProtectedFormPath)) {
          blocked.add(p)
        } else {
          publishable.add(p)
        }
    ...
    def path = paths ? paths[0] : plugins.org.craftercms.aiassistant.studio.engine.catalog.AiOrchestrationTools.repoPathFromToolInput((Map) (input ?: [:]))
    if (!path) {
      throw new IllegalArgumentException('Missing required field: path (or contentPath), or set publishScope=all|bulk|paths')
    }
    if (pathProtectFormItem && plugins.org.craftercms.aiassistant.studio.engine.context.AuthoringPreviewContext.sameRepoPath(path, normProtectedFormPath)) {
      return [
        ok                       : false,
        blockedForFormClientApply: true,
        path                     : plugins.org.craftercms.aiassistant.studio.engine.context.AuthoringPreviewContext.normalizeRepoPath(path),
        message                  :
  ...
  static List<String> collectPaths(Map input) {
    ...
    def single = plugins.org.craftercms.aiassistant.studio.engine.catalog.AiOrchestrationTools.repoPathFromToolInput(input)
    if (single?.trim()) {
      out.add(single.trim())
    }
    ...
    return out.collect {
      plugins.org.craftercms.aiassistant.studio.engine.context.AuthoringPreviewContext.normalizeRepoPath(it)
    }.findAll { it }
  }
