package plugins.org.craftercms.aiassistant.studio.engine.context
...
---"""
  ...
  /** Truncates prior-turn memory for intent-refine blocks; appends an ellipsis when over {@code maxChars}. */
  ...
  static boolean priorConversationContainsActionableContent(String fullPrompt, Map projectCfg = null) {
    ...
    if (projectCfg instanceof Map) {
      Object o = plugins.org.craftercms.aiassistant.studio.config.StudioAiAssistantProjectConfig
        .intentRecipeRoutingSection(projectCfg)?.get('priorDraftDetection')
      if (o instanceof Map) {
        detection = (Map) o
      }
    }
    return plugins.org.craftercms.aiassistant.studio.engine.routing.subrouting.PriorConversationDraftExtract
  ...
  static boolean authorCurrentRequestLooksLikeCreateFromPriorDraftFollowUp(String fullPrompt) {
    ...
    if (!plugins.org.craftercms.aiassistant.studio.engine.routing.subrouting.PriorConversationDraftExtract
  ...
  /** True when the last user+assistant pair in prior conversation was creative chat-only (not CMS field work). */
  ...
  /** True when stripped author text matches {@link #CREATIVE_LLM_ONLY} and is not an open-page inquiry. */
  ...
  /** True when the assistant reply looks like in-chat creative prose (not a page summary or CMS tool log). */
...
${asstLine}"""
  ...
  /**
   * Routing context for {@link plugins.org.craftercms.aiassistant.studio.engine.routing.subrouting.AuthoringIntentRecipeCatalog} matchers.
   */
...
---"""
  ...
  static boolean authorVisibleSuggestsIntentRecipeResearch(
    ...
    if (plugins.org.craftercms.aiassistant.studio.engine.routing.subrouting.AuthoringIntentRecipeCatalog
   ...
   * @param routingCfg {@link plugins.org.craftercms.aiassistant.studio.engine.routing.subrouting.AuthoringIntentRecipeCatalog#loadMergedCatalogRoutingConfig}
   */
