 ...
 * studio-ui {@code generatePlaceholderImageDataUrl} (grey field, centered “Sample Image” label on default size).
 * Required image-picker defaults are always at least {@link #DEFAULT_WIDTH}×{@link #DEFAULT_HEIGHT} — never 1×1.
 * <p>Custom dimensions use {@link CmsSandboxSafePng} (no AWT/ImageIO — sandbox-safe).</p>
 */
final class CmsStudioPlaceholderImage {
  ...
  /** Grey field {@code #f0f0f0} — matches prior AWT fill color. */
  private static final int PLACEHOLDER_RGB = 0xf0f0f0
  ...
  /** 150×150 grey “Sample Image” PNG used when sandbox-safe render is unavailable. */
...
private CmsStudioPlaceholderImage() {}
  ...
  /**
   * Render sample image png bytes (sandbox-safe — no {@code java.awt}).
  ...
  private static byte[] renderSampleImagePngBytes(int w, int h) {
    ...
    try {
      return CmsSandboxSafePng.solidColorPng(w, h, PLACEHOLDER_RGB)
    } catch (Throwable t) {
      log.warn(
        'renderSampleImagePngBytes: sandbox-safe PNG failed ({}x{}): {}, using embedded {}x{} fallback',
