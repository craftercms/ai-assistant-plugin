...
import plugins.org.craftercms.aiassistant.studio.contrib.tool.builtin.cms.internal.CmsContentExists
...
private AuthoringIntentRecipeEngine() {}
  ...
  private static final Set<String> READ_ONLY_TOOLS =
    plugins.org.craftercms.aiassistant.studio.engine.catalog.StudioAiToolRegistry.recipeEngineReadOnlyWireNames()
   ...
   *   confirmation tool via {@link plugins.org.craftercms.aiassistant.studio.engine.catalog.StudioAiToolRegistry#mergeRecipeConfirmationArgs}
  ...
  static Map runConfirmationStepsBlock(
    ...
    Set<String> confirmationWires =
      plugins.org.craftercms.aiassistant.studio.engine.catalog.StudioAiToolRegistry.recipeEngineConfirmationWireNames()
    Map bindings = ops.recipeEngineAuthoringBindings()
    List<Map> stepSummaries = new ArrayList<>()
    List<Map> stepResults = new ArrayList<>()
    Map<String, Map> namedSoFar = new LinkedHashMap<>()
    ...
    for (Object stepObj : steps) {
      ...
      resolvedArgs = coerceConfirmationArgStrings(
        plugins.org.craftercms.aiassistant.studio.engine.catalog.StudioAiToolRegistry.mergeRecipeConfirmationArgs(
          tool,
          resolvedArgs,
  ...
  private static Map executeReadOnlyTool(StudioToolOperations ops, String tool, Map input) {
    ...
    return plugins.org.craftercms.aiassistant.studio.engine.catalog.StudioAiToolRegistry.executeRecipePrefetchTool(
      tool,
      args,
      ops
    )
  ...
  /** Delegates to {@link plugins.org.craftercms.aiassistant.studio.engine.catalog.StudioAiToolRegistry#executeRecipeConfirmationTool}. */
  private static Map executeConfirmationTool(StudioToolOperations ops, String tool, Map input) {
    return plugins.org.craftercms.aiassistant.studio.engine.catalog.StudioAiToolRegistry.executeRecipeConfirmationTool(
      tool,
      (Map) (input ?: [:]),
      ops
    )
