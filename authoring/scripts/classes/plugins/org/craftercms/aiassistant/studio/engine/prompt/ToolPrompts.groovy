...
- **`chat_only`** — Answer in prose only; **no** repository tools this turn (greetings, thanks, praise, general Q&A, creative writing, revising prior chat text, explanations). Studio anchor metadata alone does **not** require CMS tools. For **`chat_only`**, **`turnGoal`** describes the **conversational** intent (what the author wants to hear or discuss) — not CMS repository work.
...
{"mode":"chat_only","recipeId":null,"toolName":null,"confidence":0.9,"turnGoal":"Answer the author's baseball question in natural prose.","successCriteria":null,"reason":"General conversation, not CMS work."}
{"mode":"plan","recipeId":null,"toolName":null,"confidence":0.85,"turnGoal":"Update the home page hero with today's top headline.","successCriteria":"WriteContent saved on anchored path; preview shows new copy.","reason":"Multi-step research and write on anchored page."}

Rules:
- **confidence** (0–1): compared to site **minConfidence** (often 0.55) for **recipe** mode only.
- **turnGoal**: required. For **`chat_only`**, state the author's **conversational** intent (question, thanks, praise, clarification) — not repository outcomes. For **recipe/tool/plan**, state what the author wants **accomplished on the site/repo** when done. **Never** meta-summarize as "the task involves multiple steps" or list process verbs without the author's objects (headline, page, hero image, etc.). **`reason`** is for why you picked the mode; **`turnGoal`** is what success looks like for the author.
