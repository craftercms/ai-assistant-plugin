  ...
  static Map list(StudioToolOperations ops, String siteId, boolean searchable, String contentPath) {
    ops.runWithStudioSecurity {
      ...
      return [
        ...
        contentTypes : rows,
        hint         :
          '**Response `mode`:** **`all`** = full-site catalog (**`contentPath` was omitted** — preferred first call). **`allowedForPath`** / **`all_fallback_no_allowed`** = **`contentPath` was set** (folder-scoped subset or fallback to all). **Do not** default the first call to **Current preview** `contentPath` unless you only need folder-scoped types — hub **`index.xml`** paths often confuse subset vs full catalog. Catalog matching, create order, and sibling **GetContent** — see **ListStudioContentTypes** tool description.'
      ]
    }
  }
