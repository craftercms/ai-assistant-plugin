...
| Path | Role |
|------|------|
| **`test/run-all.sh`** | **Single “all tests” script:** `bash -n`, offline **`REST_CONTRACTS_SELFTEST=1`** on **`rest-contracts.sh`**, **`yarn package`** in `sources/`, CodeRabbit on uncommitted **`sources/`** + **`authoring/scripts/`** only (`RUN_ALL_SKIP_CODERABBIT=1` to skip), then live **`rest-contracts.sh`** (JWT + JSON contracts via **`node`** / python3 / jq; `RUN_ALL_SKIP_STUDIO=1` skips Studio). Back-compat: **`test/integration/smoke.sh`** runs the same live script. |
...
| **`lib/coderabbit-review.sh`** | CodeRabbit hook for **`test/run-all.sh`** — uncommitted diffs under **`sources/`** and **`authoring/scripts/`** only. |
