...
- **Security:** MCP **`url`** values use the **same SSRF policy** as **`FetchHttpUrl`** (`plugins.org.craftercms.aiassistant.tools.http.OutboundHttpPolicy.validateUrl`). Host allowlists and disabling outbound fetch (which also blocks MCP) are **JVM-only** — see **[studio-aiassistant-jvm-parameters.md](../using-and-extending/studio-aiassistant-jvm-parameters.md)** (`aiassistant.httpFetch.*`).
